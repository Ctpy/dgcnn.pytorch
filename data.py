...
def load_data_semseg_scannet(args, partition):
    DATA_DIR = '/cluster/52/scannet'
    assert os.path.exists(DATA_DIR)
    num_points = args.num_points
    num_features = args.num_features

    #dataset_points = 16384 #65536 #32768 #16384
    dataset_points = num_points
    alternative = ''
    if args.scannet_dataset_alternative == 'fps':
        alternative = 'FPS_'
    elif args.scannet_dataset_alternative == 'subvolume':
        alternative = 'subVpoint2_'
    elif args.scannet_dataset_alternative == 'subvolume_normal':
        alternative = 'subVpoint2_normals_'
        
    train_file = os.path.join(DATA_DIR, f'scannet_train_{alternative}{dataset_points}.pickle')
    val_file = os.path.join(DATA_DIR, f'scannet_val_{alternative}{dataset_points}.pickle')
    test_file = os.path.join(DATA_DIR, f'scannet_test_{alternative}{dataset_points}.pickle')
    ...
    with open(file, 'rb') as f:
        if partition != 'test':
            ...
            assert(len(scenes) == len(labels))
            for i in range(len(scenes)):
                scene = scenes[i]
                scene_labels = labels[i]
                num_points = scene.shape[0]
                dataset_points = np.zeros(shape=[num_points, num_features], dtype=np.float32)
                dataset_points[:,:] = scene[:num_points,:num_features]
                data_batchlist.append(dataset_points)
                label_batchlist.append(scene_labels)
        else:
            scenes = pickle.load(f)
            for i in range(len(scenes)):
                scene = scenes[i]
                num_points = scene.shape[0]
                dataset_points = np.zeros(shape=[num_points, num_features], dtype=np.float32)
                dataset_points[:,:] = scene[:,:num_features]
                data_batchlist.append(dataset_points)
...
class ScanNet(Dataset):
    def __init__(self, args, num_points=2048, partition='train'):
        self.data, self.seg = load_data_semseg_scannet(args, partition)
        print(len(self.data))
        self.num_points = num_points
        self.partition = partition    
